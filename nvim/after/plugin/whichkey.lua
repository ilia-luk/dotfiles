...
local setup = {
	...
	icons = {
		...
		group = " ", -- symbol prepended to agroup
	},
	popup_mappings = {
...
local mappings = {
	...
	["z"] = { "<cmd>set cursorline! cursorcolumn!<CR>", "Cursor Highlight" },
	["f"] = {
	...
	P = {
		name = "Packages",
		b = { '<cmd>lua require("lazy").build()<CR>', "Build" },
		k = { '<cmd>lua require("lazy").check()<CR>', "Check" },
		C = { '<cmd>lua require("lazy").clean()<CR>', "Clean" },
		c = { '<cmd>lua require("lazy").clear()<CR>', "Clear" },
		...
		l = { '<cmd>lua require("lazy").load()<CR>', "Load" },
	...
	g = {
		...
		["d"] = {
			...
			["2"] = {
				"<cmd>DiffviewOpen HEAD~2<cr>",
				"HEAD~2",
			},
			["3"] = {
				"<cmd>DiffviewOpen HEAD~3<cr>",
				"HEAD~3",
			},
		},
	},
	...
	l = {
		...
		c = { "<cmd>lua require('copilot.suggestion').toggle_auto_trigger()<cr>", "Co-pilot toggle" },
		...
		["d"] = {
			name = "Diagnostics",
			f = { "<cmd> lua vim.lsp.buf.open_float()<cr>", "Open float" },
			w = {
	...
	t = {
		...
		y = { "<cmd>lua _YARN_TEST()<cr>", "Yarn test" },
		w = { "<cmd>lua _YARN_TEST_WATCH()<cr>", "Yarn test:watch" },
